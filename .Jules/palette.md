...
## 2024-06-18 - Improve mobile menu accessibility
**Learning:** Toggle buttons controlling expandable sections (e.g., mobile menus) require `aria-expanded` reflecting their state and `aria-controls` pointing to the expandable container's ID for screen readers. Furthermore, mobile menus should include an `Escape` key listener to ensure proper screen reader and keyboard support.
**Action:** Always add `aria-expanded`, `aria-controls`, and `Escape` key event listeners to mobile menu implementations.
