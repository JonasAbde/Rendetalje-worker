...
## 2024-05-18 - Accessible Mobile Menus
**Learning:** For mobile menu toggle buttons, simply adding an `aria-label` isn't enough. Screen reader users need to know if the menu is currently open or closed, and what container the button controls. Furthermore, keyboard users expect to be able to close open overlays using the `Escape` key.
**Action:** When working with togglable elements like mobile menus or dropdowns, always ensure the button has `aria-expanded` reflecting its state, `aria-controls` pointing to the ID of the menu container, and that an `Escape` key listener is attached to the document when the menu is open to provide standard keyboard accessibility.
