...
**Learning:** Added `Escape` key listener, `aria-expanded` and `aria-controls` to the mobile menu toggle button, which improves keyboard accessibility and screen reader support without changing visual styling. This is a common pattern to apply for custom dropdowns and menus.
**Action:** Always verify that interactive elements, especially custom toggles like a mobile hamburger menu, are fully accessible by keyboard (e.g., closing via Escape key) and have proper ARIA attributes to describe their state and control target.
