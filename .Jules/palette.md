...
## 2024-05-05 - Accessible Mobile Menus
**Learning:** Toggleable mobile menus require proper ARIA attributes to communicate their state to screen readers. Relying solely on visual changes is insufficient.
**Action:** Always add `aria-expanded` to reflect the menu's state and `aria-controls` to link the toggle button to the menu container's ID. Additionally, implement an `Escape` key listener to allow keyboard users to easily close the menu.
