...
## 2024-06-04 - Mobile Menu Accessibility Enhancements
**Learning:** For accessibility, mobile menu toggle buttons must include `aria-expanded` reflecting their open state and `aria-controls` pointing to the menu container's ID. Additionally, mobile menus should implement an `Escape` key listener so keyboard and screen reader users can easily close them.
**Action:** Always verify that toggleable UI elements like mobile menus have appropriate ARIA attributes connecting the trigger and the content, and ensure standard keyboard interactions (like `Escape` to close) are implemented.
