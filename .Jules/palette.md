...
## 2024-06-08 - Accessible Expandable Components
**Learning:** Toggle buttons controlling expandable sections (like mobile menus or settings panels) must explicitly convey their state and relationship to the content they control to screen reader users. Furthermore, mobile menus often trap keyboard users if they cannot easily close them without tabbing backwards.
**Action:** Always include `aria-expanded` (reflecting the true/false state) and `aria-controls` (pointing to the expandable container's ID) on the toggle button. Additionally, implement an `Escape` key listener (`keydown`) to allow keyboard users to easily dismiss the menu.
