...
**Learning:** Mobile navigation menus using an overlay or expanding layout should always support the 'Escape' key to close and explicitly declare their state to screen readers.
**Action:** Always pair `aria-expanded` and `aria-controls` on the toggle button with a matching `id` on the menu container, and include an Escape key listener.
