export const company = {
  name: "Rendetalje",
  legalName: "Rendetalje.dk ApS",
  legalForm: "ApS",
  address: "Gammel Viborgvej 40",
  postalCode: "8381",
  city: "Tilst",
  cvr: "45564096",
  phone: "+45 22 65 02 26",
  email: "info@rendetalje.dk",
  domain: "rendetalje.dk",
  url: "https://www.rendetalje.dk",
  founded: 2024,
  description:
    "Rendetalje er et professionelt rengøringsfirma i Aarhus, der tilbyder fast rengøring, flytterengøring, hovedrengøring og erhvervsrengøring i hele Aarhus og omegn.",
};

export const positioning = {
  primary:
    "Professionel rengøring i Aarhus og omegn med fokus på detaljen, klare aftaler og stabil kvalitet.",
  promise: "Vi møder op, gør arbejdet ordentligt og kommunikerer klart.",
};

export const geography = {
  areas: [
    { city: "Aarhus C", postalCode: "8000" },
    { city: "Aarhus N", postalCode: "8200" },
    { city: "Aarhus V", postalCode: "8210" },
    { city: "Risskov", postalCode: "8240" },
    { city: "Højbjerg", postalCode: "8270" },
    { city: "Viby J", postalCode: "8260" },
    { city: "Tilst", postalCode: "8381" },
    { city: "Brabrand", postalCode: "8220" },
    { city: "Hasselager", postalCode: "8361" },
  ],
  wording:
    "Vi hjælper kunder i Aarhus og omegn — herunder Aarhus C, Aarhus N, Aarhus V, Risskov, Højbjerg, Viby J, Tilst, Brabrand og Hasselager. Ved større eller faste opgaver kører vi også uden for Aarhus efter aftale.",
};

export const policies = {
  equipment: "Vi medbringer selv udstyr og svanemærkede rengøringsprodukter.",
  access:
    "Adgang aftales altid inden første besøg. Det kan ske ved, at du er hjemme, via nøgleboks, via aftalt nøgleplacering eller ved at vi opbevarer et nøglesæt efter aftale. Hvis adgangsforhold ændres, skal det meddeles inden næste planlagte besøg.",
  complaint:
    "Hvis du ikke er tilfreds med resultatet, skal du kontakte os hurtigst muligt på telefon eller mail. Vi gennemgår forholdene og aftaler en konkret udbedring, hvis der er noget, der skal rettes.",
  cancellation:
    "Afbud eller ændringer bedes meddelt hurtigst muligt og helst pr. telefon på +45 22 65 02 26. Ved ændringer samme dag skal du ringe.",
  deposit: "Vi kræver ikke depositum. For engangsopgaver betaler du efter udført arbejde, og for faste aftaler kører vi månedlig faktura.",
};
