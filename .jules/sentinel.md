...
## 2026-05-20 - [Fix IP Spoofing Risk from X-Forwarded-For]
**Vulnerability:** Rate limiting implementation relied on `X-Forwarded-For` when `CF-Connecting-IP` was missing. `X-Forwarded-For` is a client-provided header that can be trivially spoofed, allowing attackers to either bypass the rate limiter (by constantly changing IPs) or perform Denial of Service (DoS) attacks against legitimate users by blocking their IPs.
**Learning:** In Cloudflare environments, the edge guarantees the `CF-Connecting-IP` header contains the actual client IP. Fallbacks to `X-Forwarded-For` undermine this security control. Furthermore, using `crypto.randomUUID()` as a fallback for missing IPs creates a fail-open rate limit bypass.
**Prevention:** Always rely strictly on platform-guaranteed headers like `CF-Connecting-IP` for security controls like rate limiting. If the header is missing (e.g. direct origin access), fail closed by returning a static value like `unknown` to strictly rate limit unidentifiable traffic.
