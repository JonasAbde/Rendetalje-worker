...
## 2026-05-30 - XSS via Structured Data (`application/ld+json`) Injection
**Vulnerability:** The application was using `JSON.stringify(schema)` directly within the `textContent` of a `<script type="application/ld+json">` tag inside `src/components/StructuredData.tsx`. Because `JSON.stringify()` does not escape the `<` character by default, an attacker could potentially inject `</script><script>alert(1)</script>` via user-controllable inputs, such as URL paths, which bypass the JSON context and execute arbitrary JavaScript in the context of the main page.
**Learning:** `JSON.stringify()` output is not safe to embed directly inside HTML `<script>` tags, even if it's for `application/ld+json`. The browser parses `</script>` tags without understanding JSON string boundaries, leading to an escape from the script block.
**Prevention:** Always sanitize JSON before embedding it in a `<script>` tag by escaping the `<` character to `\u003c` via `.replace(/</g, "\\u003c")`.
